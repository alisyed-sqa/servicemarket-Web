/**
 * MultiWeekCleaningPage - Page Object for Multiple-Times-Per-Week Cleaning Booking
 *
 * This page object automates the complete booking flow for a cleaning service
 * that occurs MULTIPLE TIMES PER WEEK on ServiceMarket. The booking steps are:
 *   1. Navigate to the Home Cleaning service page
 *   2. Click "Book Now"
 *   3. Select options: 2 hours duration, MULTIPLE_TIMES_WEEK frequency, pick days (2 & 3)
 *   4. Select a saved address
 *   5. Pick a day and time slot
 *   6. Review and submit the booking
 *   7. Navigate back to the home screen
 *
 * Different from HomeCleaningPage because it requires selecting
 * which specific days of the week the cleaning should occur.
 */
import { Page } from '@playwright/test';
import { BasePage } from './BasePage';

export class MultiWeekCleaningPage extends BasePage {
  // --- Element Selectors ---
  ...
  readonly frequencyLabel = 'label[for="howOften-MULTIPLE_TIMES_WEEK"]';                  // "Multiple Times/Week" frequency
  readonly dayTwoLabel = 'label[for="multipleTimesWeek-2"]';                              // Day 2 checkbox for multi-day
  readonly dayThreeLabel = 'label[for="multipleTimesWeek-3"]';                            // Day 3 checkbox for multi-day
  ...
  async navigateToMultiWeekCleaning() {
    console.log('Step 5: Navigating to Multi Week Cleaning...');
  ...
  /** Step 7: Select 2 hours, MULTIPLE_TIMES_WEEK frequency, pick days 2 & 3, then click Next */
  async selectBookingOptions() {
    ...
    // Select frequency: MULTIPLE_TIMES_WEEK (cleaning multiple days per week)
    await this.page.locator(this.frequencyLabel).click();
    console.log('✓ Selected frequency: MULTIPLE_TIMES_WEEK');
    await this.page.waitForTimeout(2000);

    // Select which days of the week: Day 2 and Day 3
    await this.page.locator(this.dayTwoLabel).click();
    console.log('✓ Selected day 2');
    await this.page.waitForTimeout(2000);

    await this.page.locator(this.dayThreeLabel).click();
    console.log('✓ Selected day 3');
    await this.page.waitForTimeout(2000);
